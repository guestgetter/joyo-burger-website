...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initMobileMenu();initMobileDropdown();
    initSmoothScrolling();
...
function initMobileMenu() {
    ...
    navLinks.forEach(link => {link.addEventListener('click', function(e){if(link.classList&&link.classList.contains('mobile-dropdown-toggle')){return;}closeMenu();},{ passive: true });});
}

function initMobileDropdown(){const mobileDropdownToggles=document.querySelectorAll('.mobile-dropdown-toggle');if(!mobileDropdownToggles.length)return;mobileDropdownToggles.forEach(t=>{t.setAttribute('aria-haspopup','true');t.setAttribute('aria-expanded','false');});mobileDropdownToggles.forEach(toggle=>{toggle.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();const dropdown=this.closest('.mobile-dropdown');if(!dropdown)return;const isActive=dropdown.classList.contains('active');document.querySelectorAll('.mobile-dropdown').forEach(otherDropdown=>{if(otherDropdown!==dropdown){otherDropdown.classList.remove('active');const otherToggle=otherDropdown.querySelector('.mobile-dropdown-toggle');if(otherToggle)otherToggle.classList.remove('active');}});dropdown.classList.toggle('active',!isActive);this.classList.toggle('active',!isActive);this.setAttribute('aria-expanded',!isActive?'true':'false');},{ passive: false });});}
/**
